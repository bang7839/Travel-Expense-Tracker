...
2. **🧳 登入畫面多行程與首頁切換**
   ...
   - 登入頁直接提供「➕ 建立新旅遊行程」按鈕。
3. **🔐 管理者 (Owner) vs 成員 (Member) 嚴格權限控管**
   - **管理者 / 擁有者 (Owner)**：輸入管理者密碼解鎖（預設 `1234`），**只有管理者才可以「建立新行程」、「刪除行程」**，以及修改行程名稱、匯率、成員名單、雙層密碼與信用卡。
   - **一般成員 (Member)**：輸入成員密碼解鎖（預設 `0000`），**僅能新增消費與查看結算**。成員無權限建立新行程、刪除行程或開啟系統設定。
4. **📎 備註附件 / 收據相片上傳與檢視 (新增功能)**
   - 記帳時點擊「📷 拍照 / 選擇檔案」，支援上傳收據、發票照片或 PDF/Word 附件。
   - 上傳前提供即時縮圖與檔案預覽，支援大於 5MB 大小防護。
   - 在消費紀錄列表中，有附件的項目會自動顯示 🖼️ **相片** / 📎 **附件** 晶片按鈕。
   - 點擊即可開啟**全螢幕多媒體檢視器**，圖片可直接高清放大瀏覽，並提供一鍵「⬇️ 下載附件」保存至手機！
5. **🌐 全域 Google Sheet API**
   - 設定中的 Google API 網址升級為全域設置，所有建立的行程自動共用同一個 Google 試算表（附件同步儲存）。
3. **💴 多幣別與動態匯率換算**
